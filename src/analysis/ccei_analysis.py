...
def global_ccei_analysis(df, match_id=None, 
                                set_num=None, 
                                start_rally_id=None, 
                                end_rally_id=None,
                                event_type=None,
...
def formation_change_ccei_analysis(df, match_id=None, 
                                set_num=None, 
                                start_rally_id=None, 
                                end_rally_id=None,
                                event_type=None,):
    ...
    transition_probs.index = ['No Coverage (CCEI=False)', 'With Coverage (CCEI=True)']
    print(transition_probs.round(2).to_string())

    def draw_stacked_bar(ct):
        categories = ['No Coverage(CCEI = 0)', 'With Coverage(CCEI = 1)']
        
        def get_counts(col_name):
            ccei_false = ct.loc[ct.index.isin([False, 0, 0.0]), col_name].sum() if col_name in ct.columns else 0
            ccei_true = ct.loc[ct.index.isin([True, 1, 1.0]), col_name].sum() if col_name in ct.columns else 0
            return np.array([ccei_false, ccei_true])
        ...
        os.makedirs('./img/Coordination Analysis', exist_ok=True)
        plt.savefig('./img/Coordination Analysis/CCEI_Formation_trans_probs.pdf', dpi=400)
        plt.show()
